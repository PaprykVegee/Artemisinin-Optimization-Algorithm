...
$$min \sum_{i=1}^{n} \sum_{j=1}^{n} a_{ij} \cdot b_{p(i)p(j)}$$

Where:
* $n$: The size of the instance (number of facilities/locations).
* $A = [a_{ij}]$: The **Flow Matrix**, where $a_{ij}$ is the flow between facility $i$ and facility $j$.
* $B = [b_{ij}]$: The **Distance Matrix**, where $b_{ij}$ is the distance between location $i$ and location $j$.
